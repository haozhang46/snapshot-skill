...
def test_snapshot_creates_tag(tmp_path: Path) -> None:
    repo_path = init_repo(tmp_path)
    tm = TimeMachine(repo_path)
    result = tm.snapshot(reason="update config", level="high")

    assert result["snapshot_id"]
    assert result["git_commit"]
    assert any(tag.startswith("pre-change/") for tag in result["tags"])
